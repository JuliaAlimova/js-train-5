...
function sumArray(array) {
  let sumOfValuesArray = null;
  for (const key in array) {
    sumOfValuesArray += array[key];
  }
  return sumOfValuesArray;
...
function multiplyArray(array) {
  let multOfValuesArray = 1;
  for (const key of array) {
    multOfValuesArray *= key;
  }
  return multOfValuesArray;
...
function findMinMax([firstElement, ...restElements]) {
  let minVal = firstElement;
  let maxVal = firstElement;
  for (let i = 0; i < restElements.length; i++) {
    const valueOfArray = restElements[i];
    if (valueOfArray < minVal) {
      minVal = valueOfArray;
    } else if (valueOfArray > maxVal) {
      maxVal = valueOfArray;
    }
  }
